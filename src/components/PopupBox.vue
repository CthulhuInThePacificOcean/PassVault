  ...
  <div class="popup-box">
    <div class="popup">
      <div class="content">
        ...
        <header>
          <p v-if="popupBoxUpdate" class="title">Update PassCard</p>
          <p v-else class="title">Add a new PassCard</p>
        </header>
        <form action="#">
          <div class="row-name">
            <label>Website or Application Name: </label>
            <input type="text" class="name-input file-name" v-model="name" />
          </div>
          <div class="row-website">
            <label>Link to website: </label>
            <input type="text" class="site-input file-name" v-model="website"/>
          </div>
          <div class="row-username">
            <label>Username: </label>
            <input type="text" class="user-input file-name" v-model="username" />
          </div>
          <div class="row-password">
            <label>Password: </label>
            <input type="text" class="pass-input file-name" v-model="password" />
            <p style="font-size: 15px; margin: 0;">Don't have an existing password for the account? Go to <span @click="this.$router.push({name: 'generator'})">Password Generator</span></p>
          </div>
          <div class="row-color">
            <label>Color</label>
            <input type="color" class="color-input" v-model="color" />
          </div>
          <button @click.prevent="onSubmit" v-if="popupBoxUpdate" class="auth-btn" style="margin-bottom: 5px; margin-left: 5px;">Update PassCard</button>
          <button @click.prevent="onSubmit" v-else class="auth-btn" style="margin-bottom: 5px; margin-left: 5px;">Add PassCard</button>
        </form>
      </div>
    </div>
  </div>
